<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800">Edit Jadwal</h2>
    </x-slot>
    <div class="py-12">
            ...
            <form method="POST" action="{{ route('admin.schedules.update', $schedule) }}"
                ...
                <div><x-input-label for="course_id" value="Mata Kuliah" /><select id="course_id" name="course_id"
                        ...
                        @foreach ($courses as $course)
                            <option value="{{ $course->id }}" @selected(old('course_id', $schedule->course_id) == $course->id)>{{ $course->kode_matkul }}
                                - {{ $course->nama }}</option>
                        @endforeach
                    </select>
                ...
                <div><x-input-label for="hari" value="Hari" /><select id="hari" name="hari"
                        ...
                        @foreach (['senin', 'selasa', 'rabu', 'kamis', 'jumat', 'sabtu', 'minggu'] as $day)
                            <option value="{{ $day }}" @selected(old('hari', $schedule->hari) === $day)>{{ ucfirst($day) }}
                            </option>
                        @endforeach
                ...
                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2">
                    <div><x-input-label for="jam_mulai" value="Jam Mulai" /><x-text-input id="jam_mulai"
                            name="jam_mulai" type="time" class="mt-1 block w-full"
                            value="{{ old('jam_mulai', substr($schedule->jam_mulai, 0, 5)) }}" required /><x-input-error
                            :messages="$errors->get('jam_mulai')" class="mt-2" /></div>
                    <div><x-input-label for="jam_selesai" value="Jam Selesai" /><x-text-input id="jam_selesai"
                            name="jam_selesai" type="time" class="mt-1 block w-full"
                            value="{{ old('jam_selesai', substr($schedule->jam_selesai, 0, 5)) }}"
                ...
                <div><x-input-label for="ruangan" value="Ruangan" /><x-text-input id="ruangan" name="ruangan"
                        class="mt-1 block w-full" value="{{ old('ruangan', $schedule->ruangan) }}"
                        required /><x-input-error :messages="$errors->get('ruangan')" class="mt-2" /></div>
